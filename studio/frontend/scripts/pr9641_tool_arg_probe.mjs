...
// The REAL helper from the revision under test, so this probe cannot pass by
// testing its own copy. Revisions before the helper existed fall back to the
// inline coercion they actually shipped.
let toolArgText;
let helperSource;
try {
  ({ toolArgText } = await import(
    new URL("../src/components/assistant-ui/tool-arg-text.ts", import.meta.url).href
  ));
  helperSource = "the checked-out src/components/assistant-ui/tool-arg-text.ts";
} catch {
  toolArgText = (value) => (value == null ? "" : String(value));
  helperSource = "no helper in this revision; using the inline String() it shipped";
}
console.log(`helper: ${helperSource}\n`);
...
const CASES = [
  ...
  {
    // Valid JSON: the own property shadows Object.prototype's and is not
    // callable, so String() throws "Cannot convert object to primitive value".
    file: "tool-ui-python.tsx",
    from: "const code =",
    to: "const isRunning =",
    run: "code.split('\\n')[0] ?? ''",
    args: JSON.parse('{"code":{"toString":null}}'),
    expect: '{"toString":null}',
    what: "python: an object whose own toString is not callable",
  },
  {
