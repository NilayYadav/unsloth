...
class InferenceBackend:
    ...
    def _generate_vision_response(
        ...
        if attached:
            # Profiled from the processor that renders this turn, so a vision request
            # is gated on the loaded model exactly as the text path is (#7066).
            ...
            # A conversation that marks its own images is rendered whole: collapsing it
            # to the last user turn would drop the tool exchange the answer depends on.
            shapes = []
            if _count_image_markers(messages):
                history = _history_vision_messages(messages, system_prompt)
                if image is not None:
                    # Marked on the newest user turn, so its pixels come after the
                    # ones history already marks.
