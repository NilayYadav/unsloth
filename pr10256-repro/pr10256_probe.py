...
RECORDED = [
    (4032, 3024, "edit", "Edit a 4032x3024 phone photo: the edit branch snaps to /16 and does not clamp"),
    (1024, 208, "img2img", "Transform a 1920x400 source with the sliders at 1024: _fit_within then snap"),
    (4096, 4096, "edit", "decode_b64_image's 4096 ceiling, the largest record reachable"),
    (2048, 256, None, "already inside the schema"),
    (1024, 1024, None, "a plain Create"),
]


def restored_size(width, height, workflow):
    out = subprocess.run(
        [
            "node", "--experimental-strip-types", "--no-warnings",
            str(Path(__file__).with_name("restore_size_probe.mjs")),
            str(ROOT), str(width), str(height), str(workflow),
        ],
...
def main():
    ...
    print()
    failures = []
    for width, height, workflow, why in RECORDED:
        restored = restored_size(width, height, workflow)
        ...
        ratio_out = rw / rh
        print(f"recorded {width}x{height}  workflow={workflow}  ({why})")
