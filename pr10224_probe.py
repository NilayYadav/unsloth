...
# The Settings -> Resources tab reads the per-device figure from /api/system,
# which falls back to total - used for any device that does not report free.
print("")
print("=== the device row the Settings -> Resources tab renders ===")
util = H.get_visible_gpu_utilization()
row = (util.get("devices") or [{}])[0]
summary_free = H.get_gpu_summary().get("vram_free_gb")
subtraction = None
if row.get("vram_total_gb") is not None and row.get("vram_used_gb") is not None:
    subtraction = round(row["vram_total_gb"] - row["vram_used_gb"], 2)
print("  device vram_total_gb=%s vram_used_gb=%s vram_free_gb=%s"
      % (row.get("vram_total_gb"), row.get("vram_used_gb"), row.get("vram_free_gb")))
print("  /api/system/hardware gpu.vram_free_gb = %s" % summary_free)
print("  total - used (what /api/system falls back to) = %s" % subtraction)
check("resources tab", "the device row reports free instead of leaving it to be subtracted",
      row.get("vram_free_gb") is not None,
      "device vram_free_gb=%s; /api/system would show %s GB free next to a picker using %s GB"
      % (row.get("vram_free_gb"), subtraction, summary_free))
check("resources tab", "the tab and the training-method picker read the same number",
      row.get("vram_free_gb") is not None and summary_free is not None
      and abs(row["vram_free_gb"] - summary_free) <= 0.05,
      "tab=%s vs picker=%s" % (row.get("vram_free_gb"), summary_free))
